...
## 20:00 - CloudWatch 모듈 확장 및 문서 업데이트

### 변경 사항

- 수정 파일: `modules/cloudwatch/main.tf`
  - ECS, EC2, Lambda, VPC Flow Logs, Application 로그 그룹 추가
  - 재사용성을 위해 다양한 서비스 지원

- 수정 파일: `modules/cloudwatch/variables.tf`
  - 각 서비스별 생성 플래그 및 보존 기간 변수 추가
  - `lambda_functions`, `application_log_groups` 동적 변수

- 수정 파일: `modules/cloudwatch/outputs.tf`
  - 모든 서비스별 로그 그룹 출력 추가

- 수정 파일: `docs/README.md`
  - 디렉토리 구조에 05-cloudwatch 레이어 추가
  - 삭제 순서 업데이트

- 수정 파일: `docs/architecture.md`
  - 레이어 배포 순서 다이어그램 업데이트
  - 레이어별 설명 테이블에 05-cloudwatch 추가

### 모듈 확장 이유

- ECS, EC2 등 추후 다른 서비스에서도 CloudWatch 로그 그룹 재사용
- 중앙 집중화된 로그 관리 및 일관된 설정 적용
